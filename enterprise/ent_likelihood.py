...
class Likelihood:
    ...
    def __init__(self,
        ...
        self.Tspans = jnp.array([toa[-1] - toa[0] for toa in self.toas])
        self.Ntinvs = Ntinvs
        self.Fs = Fs
        self.Ntoas = Ntoas
        self.Np = Np
        ...
        self.rho_scales = jnp.array([(c.year_sec ** 3.) / (12. * (jnp.pi ** 2.) * Tspan)
                                     for Tspan in self.Tspans])
        self.rho_scales1 = np.array(self.Tspans / c.year_sec)
        self.arrs = np.array([jnp.repeat(jnp.arange(1, self.Na // 2 + 1), 2) for _ in range(self.Np)],
                             dtype=np.float64)
        self.arrs /= self.rho_scales1[:, None]
        self.arrs = jnp.array(self.arrs)

        self.vectorized_get_rho_diag_full = jit(vmap(self.get_rho_diag, in_axes=(0, 0, 0)))
    ...
    def get_rho_diag(self, hyper_params, rho_scale, arr):
    ...
    @partial(jit, static_argnums=(0,))
    def vectorized_get_rho_diag(self, hyper_params_all):
        return self.vectorized_get_rho_diag_full(hyper_params_all, self.rho_scales, self.arrs)
