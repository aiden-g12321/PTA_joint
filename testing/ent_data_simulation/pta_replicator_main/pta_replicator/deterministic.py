...
def create_gw_antenna_pattern(pos, gwtheta, gwphi):
    ...
    sgwphi = np.sin(gwphi)
    cgwphi = np.cos(gwphi)
    sgwtheta = np.sin(gwtheta)
    cgwtheta = np.cos(gwtheta)

    m = np.array([sgwphi, -cgwphi, 0.0])
    n = np.array([-cgwtheta * cgwphi, -cgwtheta * sgwphi, sgwtheta])
    omhat = np.array([-sgwtheta * cgwphi, -sgwtheta * sgwphi, -cgwtheta])

    mdotpos = np.dot(m, pos)
    ndotpos = np.dot(n, pos)
    omhatdotpos = np.dot(omhat, pos)
...
def add_cgw(psr, CW_params, psr_params, toas_input, pos):
    ...
    # unpack parameters
    log10_mc, log10_fgw, cos_inc, psi, log10_dist, cos_gwtheta, gwphi, phase0 = CW_params
    pdist, p_phase = psr_params

    # convert units to time
    mc = np.power(10., log10_mc) * Tsun
    fgw = np.power(10., log10_fgw)
    gwtheta = np.arccos(cos_gwtheta)
    inc = np.arccos(cos_inc)
    p_dist = pdist * kpc / c
    dist = np.power(10., log10_dist) * Mpc / c

    psr.update_added_signals('{}_'.format(psr.name)+'cw',
                             {'gwtheta': gwtheta,
                              'gwphi':gwphi,
                              'mc':mc,
                              'dist':dist,
    ...
    fplus, fcross, cosMu = create_gw_antenna_pattern(pos, gwtheta, gwphi)

    # get pulsar time
    toas_copy = np.copy(toas_input)
    toas_copy -= tref
    tp = toas_copy - p_dist*(1-cosMu)
